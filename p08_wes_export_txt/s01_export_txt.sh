...
# Prepare script for html report (v2)
r_script_to_check_tables="library('rmarkdown', lib='"${r_lib_folder}/"'); render('"${scripts_folder}"/r02_check_update_tables_html_v2.Rmd', params=list(dataset='"${dataset}"', working_folder='"${export_folder}"', vv_file='"${VV}"', kgen_file='"${kgen}"', exac_file='"${exac}"', gt_file='"${GT}"', gt_add_file='"${GT_add}"', gt_dom_file='"${GT_dom}"', gt_rec_file='"${GT_rec}"', dp_file='"${DP}"', ad_file='"${AD}"', gq_file='"${GQ}"', pl_file='"${PL}"'), output_file='"${check_html}"')"
