...
annotations_for_vqsr_model=$(get_parameter "annotations_for_vqsr_model") # e.g. QD MQ MQRankSum ReadPosRankSum FS SOR InbreedingCoeff

annotations=""
for annotation in ${annotations_for_vqsr_model}
do
  annotations="${annotations} -an ${annotation}"
done # next annotation
...
r_bin_folder=$(get_parameter "r_bin_folder") # e.g. r/R-3.3.2/bin/
r_bin_folder="${tools_folder}/${r_bin_folder}" # for Rmarkdown reports

r_lib_folder=$(get_parameter "r_lib_folder") # e.g. r/R-3.3.2/lib64/R/library
